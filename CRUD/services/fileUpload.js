...
const { profile_image } = require("./config");
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.resolve(filePath + `/${profile_image[file.fieldname]}`))
  },
  filename: function (req, file, cb) {

    const finalName = Date.now() + path.extname(file.originalname)
    const folderCreate = path.resolve(__dirname + "./../" + folderName + `/${profile_image[file.fieldname]}`)

    if (!fs.existsSync(folderCreate)) fs.mkdirSync(folderCreate)
    req.body[file.fieldname] = `${profile_image[file.fieldname]}/${finalName}`
